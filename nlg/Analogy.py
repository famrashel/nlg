...
from nlg.cpa_solver import cpa_solver
...
def solve_analogy(term_A, term_B, term_C, mode='c'):
	term_D = []
	if mode == 'cpa': # TO BE FIXED: there is still a problem with the cpa solver
		term_D.extend(list(cpa_solver.Analogy(term_A, term_B, term_C).Solve()))
	else:	# default using c_module
		term_D.append(solvenlg(term_A, term_B, term_C))
	return term_D
