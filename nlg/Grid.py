...
__date__, __version__ = '01/05/2016', '0.1' # Creation
__date__, __version__ = '15/06/2017', '1.0' # Class: NlgGrid
__date__, __version__ = '21/05/2020', '1.1' # Update read_argv() and main() to fit the newest version of Python
__date__, __version__ = '15/12/2021', '1.2' # Update ListOfGrids.statistics()
											# Grid.attributes is now a dictionary for better readability
...
class Grid(list):
	...
	def checkninsert(self, cluster, saturation_threshold):
		...
		if insertable:
			if saturation_threshold == 0:
				...
				if newGrid.attributes['saturation'] >= saturation_threshold:
					self.insert(cluster, details[0], details[1], details[2], details[3], details[4])
					if details[0]:
						if __trace__: print('### Inserted - row', details, file=sys.stderr)
						return (True, 'row')
	...
	def predictable_words(self):
		...
		if self.attributes['saturation'] == 1: return	# Complete grid (no holes)
		for i in range(len(self)):
			for j in range(len(self[i])):
				if self[i][j] is None:
...
class ListOfGrids(list):
	...
	def fromClusters(cls, clusters, saturation=float(0.0)):
		...
		from nlg.nlgGrid.nlgclu2grid import clus2grids
		return cls(clus2grids(clusters, saturation))
	
	@classmethod
	def fromVectors(cls, vectors, min_clu_size=2, max_clu_size=None, saturation=float(0.0)):
		"""
		Build list of analogical grids from list of analogical clusters.
		"""
		...
		# return cls(nlgclus2grids(clusters, saturation))
		from nlg.nlgGrid.nlgclu2grid import clus2grids
		return cls(clus2grids(list_of_strclusters, saturation))
	...
	def statistics(self, print_detail=False):
		"""
		...
		for grid in self:
			grid.set_attributes()
			size_saturation_freq[(grid.attributes['size'], grid.attributes['saturation'])] += 1
			sizes.append(grid.attributes['size'])
			saturations.append(grid.attributes['saturation'])
		...
		print(f">> # of grids: {len(self)}")
		print(f">> Avg grid size: {(sum(sizes)/float(len(self))):.2f}")
		print(f">> Avg grid saturation: {(sum(saturations)/float(len(self))):.3f}")
...
def main(file=sys.stdin):
	list_of_grids = ListOfGrids.fromFile(file)
	if options.statistics and len(list_of_grids) > 0:
		if options.verbose: print('# Calculating some statistics ...', file=sys.stderr)
		list_of_grids.statistics()
