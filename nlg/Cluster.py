...
def visualize(dist, nlg):
	import matplotlib
	matplotlib.use('TKAgg') # Comment this line if there is an error caused by matplotlib's kernel
...
class Cluster(list):
	...
	def is_empty_intersection(self, other):
		...
		return selfAs.intersection(otherAs) == set() \
		   and selfAs.intersection(otherBs) == set() \
		   and selfBs.intersection(otherAs) == set() \
		   and selfBs.intersection(otherBs) == set()
...
class ListOfClusters(list):
	...
	def fromVectors(cls, vectors, **kwargs):
		...
		"""
		from .nlgCluster.nlgclu import NlgClusteringFromVectors
	...
	def statistics(self, visualization=__visualization__):
...
#					break
	...
	def __repr__(self):
		...
		self[:] = sorted(self, key=len, reverse=True)
		return '\n'.join(
			( [] if len(self.indistinguishables) else ['{}'.format(self.indistinguishables) ]) +
